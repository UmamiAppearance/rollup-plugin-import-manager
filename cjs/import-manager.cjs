...
class ImportManager {
    ...
    constructor(source, filename, warnSpamProtection, autoSearch=true) {
        ...
        this.parsedCode = acorn.parse(source, this.parseOptions);
    ...
    es6NodeToUnit(node) {
        ...
        if (typeof node === "string") {
            code = new MagicString__default["default"](node);
            node = acorn.parse(node, this.parseOptions);
        } else {
            code = new MagicString__default["default"](this.code.slice(node.start, node.end));
        }
